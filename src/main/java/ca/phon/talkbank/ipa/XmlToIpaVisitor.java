...
public class XmlToIpaVisitor extends VisitorAdapter<Object> {
    ...
    public void visitStress(StressType stressType) {
        final ca.phon.ipa.StressType type = switch (stressType.getType()) {
            case PRIMARY -> ca.phon.ipa.StressType.PRIMARY;
            case SECONDARY -> ca.phon.ipa.StressType.SECONDARY;
        };
    ...
    public void visitCompoundPhone(CompoundPhoneType CompoundPhoneType) {
        ...
        for(JAXBElement<?> ele:CompoundPhoneType.getContent()) {
            if(!ele.getName().getLocalPart().equals("lig")) {
                ...
                final JAXBElement<LigatureType> ligEle = (JAXBElement<LigatureType>) ele;
